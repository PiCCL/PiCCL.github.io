...
\r
const clip = PICCL.intersect(shape, bar).pointSnap(bar, shape, {anchor: [0,0]})\r
\r
const outline = PICCL.union([clip, border]).pointSnap(clip, border)\r
\r
const label = new PICCL.Text({fontSize: 50, textAnchor: "middle"})\r
    .mapValue("content", "category")\r
const barWithLabel = PICCL.union([outline, label])\r
    .pointSnap(label, outline, {anchor: [0.5, 0], offset: [0, -70]})\r
